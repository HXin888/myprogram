...
public class StuAction {
    ...
    public String addStu(Cls cls, Stu stu, MultipartFile mfile, BindingResult result, ModelMap modelMap, RedirectAttributes attr, HttpSession session) {

        System.out.println(mfile);
        System.out.println("," + stu.getSage() + "," + stu.getSname() + "," + stu.getSphone()+","+cls.getCno());
        cls = clsService.getClsById(cls.getCno());
        if(result.hasErrors()){
        ...
        if (stu != null) {
            if(mfile != null) {
                if (!mfile.isEmpty()) {
                    stu = upLoadFile(stu, mfile, session);
                }
            }
            stu.setCls(cls);
            System.out.println(stu.getSage() + "," + stu.getSname() + "," + stu.getSphone());
    ...
    public String updateStu( Stu stu, MultipartFile file, HttpSession session, RedirectAttributes attr) {
        if(file != null){
            if (!file.isEmpty()) {
    ...
    //文件上传
    private Stu upLoadFile(Stu stu, MultipartFile mfile, HttpSession session) {
        //获取服务器路径
        ...
        try {
            ...
            mfile.transferTo(new File(basePath + "/" + fileName));
